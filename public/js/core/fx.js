...
export function generateTeach(task) {
  ...
  const explains = {
    ...
    '2-6b': '还记得教材里的黑灯工厂吗？关着灯也能干活，靠的是数据不停转圈——就像血液循环，工厂才真正「活」起来。这套循环就是数据闭环：感知→传输→分析→决策→执行。再看两张模型：功能视角 IIRA 四层（边缘/平台/应用/业务）、部署视角 ISA-95 五层（L0现场/L1控制/L2监控/L3运营/L4业务）——这是全书的「地图」，后面装系统、调协议、上云、看板都不迷路。',
...
'3-7': 'Modbus TCP 读取数据用 modbus read 命令，参数依次是 IP 地址、端口号、寄存器地址。Holding Register 是最常用的寄存器类型，存储传感器当前值。',
    ...
    '4-8': '数据在设备旁边直接算叫边缘计算，几百公里外算叫云计算。前者延迟低响应快，后者算力强存量大。哪些任务等不起几毫秒延迟，必须就地处理？ 工业互联网常用『云-边-端』三层架构：云端集中存储和大数据分析，终端（传感器/PLC）负责采集，边缘就在设备旁边就近计算、毫秒级响应。边缘计算说的就是中间这层——数据就地处理，不过度依赖云端。',
        '5-6b': '刚才 Docker 把应用打包跑起来了，先别急着写代码——先看懂「平台」是干嘛的。一句话：它是「向下连设备、向上承载应用」的中间层，就像快递调度中心——向下「接单」（把车间设备的方言翻译成普通话），向上「派送」（把工业APP跑起来给工人用），中间再存数据、算数据、喂AI。国内 240+ 家平台看着眼花，其实只有两种出身：互联网公司往下沉（拿云算力+AI 做输出，如阿里云IoT），制造企业往上长（从自家工厂长出来，如树根互联、卡奥斯）。其余记不住没关系，六个字必须刻进脑子：连设备、承载应用。',
