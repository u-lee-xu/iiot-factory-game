...
export function generateTeach(task) {
  ...
  const explains = {
    ...
    '1-5b': '刚才排了 OSI 七层，现在记实际干活用的精简版——TCP/IP 四层：应用层(面对用户，HTTP/DNS)、传输层(可靠传输，TCP/UDP)、网络层(找路，IP/路由器)、网络接口层(收发字节，网卡/交换机)。记成快递：业务员→干线→分拣→配送。',
    ...
    '7-0': '厂里几十台设备都要送数据，用 Modbus 一问一答会累死主站——就像挨个点名。MQTT 换个思路：设备把消息「发布」到 Broker（相当于在群里发消息），要数据的人「订阅」这个主题（相当于关注群），Broker 自动转发。一对多、解耦、省资源。Mosquitto 就是这样一个 Broker，apt install 就能装。',
    ...
    '2-6b': '先把全厂的「地图」记清楚：所有工业互联网场景本质都是数据闭环——感知→传输→分析→决策→执行。再看两张模型：功能视角用 IIRA 四层（边缘/平台/应用/业务），部署视角用 ISA-95 五层（L0现场/L1控制/L2监控/L3运营/L4业务）。心里有这张地图，后面装系统、调协议、上云、看板就都不迷路。',
